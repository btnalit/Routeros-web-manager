...
export async function createConfig(req: Request, res: Response): Promise<void> {
  try {
    const input: CreateAPIConfigInput = req.body;
...
export async function updateConfig(req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params;
    const updates: UpdateAPIConfigInput = req.body;
...
export async function chat(req: Request, res: Response): Promise<void> {
  try {
    const { configId, sessionId, message, includeContext = true } = req.body;
...
export async function chatStream(req: Request, res: Response): Promise<void> {
  try {
    ...
    try {
      ...
      // 保存助手消息到会话
      if (sessionId) {
        await chatSessionService.addMessage(sessionId, { role: 'assistant', content: fullContent });
      }
 ...
 */
export async function getContextSections(_req: Request, res: Response): Promise<void> {
  try {
...
export async function executeScript(req: Request, res: Response): Promise<void> {
  try {
    const { script, sessionId, dryRun = false } = req.body;
...
export async function createSession(req: Request, res: Response): Promise<void> {
  try {
    const { provider, model, configId } = req.body;
    ...
    const session = await chatSessionService.create(sessionProvider, sessionModel);
