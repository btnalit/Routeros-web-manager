...
export class AlertPreprocessor implements IAlertPreprocessor {
  ...
  async enrichContext(event: UnifiedEvent): Promise<UnifiedEvent> {
    try {
      ...
      logger.warn('Failed to enrich event with device info:', error);
    }
  ...
  private async getDeviceInfo(): Promise<DeviceInfo | null> {
    ...
    try {
      // Get system identity
      const identityResult = await routerosClient.print<{ name: string }>(
        '/system/identity'
      );
      const hostname = identityResult[0]?.name || 'unknown';

      // Get system resource info
      const resourceResult = await routerosClient.print<{
        'board-name'?: string;
        version?: string;
