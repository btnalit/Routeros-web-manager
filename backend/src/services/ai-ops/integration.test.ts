...
describe('Integration: Multi-Source Correlation', () => {
  ...
  it('should correlate events from different sources', async () => {
    ...
    const analysis = await rootCauseAnalyzer.analyzeCorrelated(events, 60000);
    expect(analysis.rootCauses.length).toBeGreaterThan(0);
    expect(analysis.timeline.events.length).toBe(2);
  });

  it('should generate timeline in chronological order', () => {
    const now = Date.now();
    const events: UnifiedEvent[] = [
      { id: 'e3', source: 'metrics', timestamp: now + 2000, severity: 'warning', category: 'system', message: 'Third', rawData: {}, metadata: {} },
      { id: 'e1', source: 'syslog', timestamp: now, severity: 'critical', category: 'system', message: 'First', rawData: {}, metadata: {} },
      { id: 'e2', source: 'metrics', timestamp: now + 1000, severity: 'warning', category: 'system', message: 'Second', rawData: {}, metadata: {} },
    ];

    const timeline = rootCauseAnalyzer.generateTimeline(events);
    expect(timeline.events[0].eventId).toBe('e1');
    expect(timeline.events[1].eventId).toBe('e2');
    expect(timeline.events[2].eventId).toBe('e3');
  });
});
