...
export class ConfigSnapshotService implements IConfigSnapshotService {
  ...
  private async exportConfig(): Promise<string> {
    ...
    try {
      ...
      if (Array.isArray(response) && response.length > 0) {
        // 响应可能是数组形式
        const exportData = response[0] as Record<string, unknown>;
        if (exportData && typeof exportData === 'object') {
          // 尝试获取导出内容
          if ('ret' in exportData && typeof exportData.ret === 'string') {
            return exportData.ret;
          }
          // 如果是其他格式，序列化为字符串
  ...
  async createSnapshot(trigger: SnapshotTrigger): Promise<ConfigSnapshot> {
    ...
    if (this.snapshots.length >= 2 && trigger !== 'pre-remediation') {
      try {
        ...
        const diff = await this.compareSnapshots(previousSnapshot.id, snapshot.id);
   ...
   * 获取快照列表
   */
  async getSnapshots(limit?: number): Promise<ConfigSnapshot[]> {
    await this.initialize();
  ...
  async restoreSnapshot(id: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      // 在恢复前创建一个快照作为回滚点
      const preRestoreSnapshot = await this.createSnapshot('pre-remediation');
