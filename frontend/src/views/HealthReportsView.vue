  ...
  <div class="health-reports-view">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <span class="header-title">健康报告</span>
      </div>
    ...
    <el-card v-else shadow="hover">
      ...
      >
        ...
        <el-table-column label="健康状态" width="120">
          <template #default="{ row }">
            <el-tag :type="getHealthType(row.summary.overallHealth)" size="small">
...
const generateReport = async () => {
  ...
  if (generateForm.value.periodType === 'day') {
    ...
    to = now
  } else if (generateForm.value.dateRange) {
  ...
  try {
    const response = await reportsApi.generate(
      from,
      to,
      generateForm.value.channelIds.length > 0 ? generateForm.value.channelIds : undefined
    )
    if (response.data.success && response.data.data) {
