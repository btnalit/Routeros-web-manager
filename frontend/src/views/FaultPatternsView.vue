  ...
  <div class="fault-patterns-view">
    ...
    <div class="page-header">
      <div class="header-left">
        <el-icon :size="28" color="#67c23a"><FirstAidKit /></el-icon>
        <span class="header-title">故障自愈管理</span>
      </div>
...
import { Refresh, Plus, Delete, FirstAidKit, QuestionFilled } from '@element-plus/icons-vue'
...
const loadPatterns = async () => {
  ...
  try {
    const response = await faultPatternsApi.getAll()
...
const loadRemediationHistory = async () => {
  ...
  try {
    const response = await remediationsApi.getAll(50)
    if (response.data.success && response.data.data) {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditing.value && editingPatternId.value) {
      ...
      await faultPatternsApi.create(data)
